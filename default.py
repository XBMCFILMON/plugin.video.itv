...
import datetime
import time
ADDON = xbmcaddon.Addon(id='plugin.video.itv')
...
def CATS():
        ...
        addDir('ITV1 Live','http://www.itv.com/ukonly/mediaplayer/xml/channels.itv1.xml',7,foricon+'art/1.png',isFolder=False)
...
def PLAY_STREAM(name,url):
    ...
    if (quality == 0):
        ...
        playpath = playpath[2]
    if name=='ITV1 Live':
        iconimage =foricon+'art/1.png'
    if name=='ITV4 Live':
        iconimage =foricon+'art/4.png'
    url=rtmp[0]+' playpath='+playpath+' swfUrl=http://www.itv.com/mediaplayer/ITVMediaPlayer.swf?v=12.18.4 live=true timeout=10 swfvfy=true'
...
def addDir(name,url,mode,iconimage,plot='',isFolder=True):
        ...
        if mode==2:
            try:
                if name in FAV:
                    menu.append(('Remove Favorites','XBMC.Container.Update(%s?name=%s&mode=14&iconimage=None&url=None)' %(sys.argv[0],name)))
                else:
